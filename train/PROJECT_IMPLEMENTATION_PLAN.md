...
| `stock-schedule-batch.html` | 系列开班（二级页面） |
...
| `ops-pre-notice.html` | 行前提醒（列表页；提醒配置为弹窗） |
...
- 发团作业：发团看板 / 行前提醒 / 证件复核；行前提醒配置在行前提醒列表中以弹窗打开。
- 变更协同：变更协同。
...
6. 履约计调：团队列表 → 名单总览 → 下达计划 → 发团看板 → 证件复核。
